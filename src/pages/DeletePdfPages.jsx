...
const DeletePdfPages = () => {
    ...
    if (process.env.NODE_ENV === 'development') {
      if (deletePdfPagesConfig.metadata?.schemaVersion === '2.0.0') {
        ...
        if (!result.isValid) {
          ...
          console.log('✅ DeletePdfPages config is valid (Schema v2.0)');
        }
      }
    }
  }, []);
  ...
  const content = useMemo(() => {
    return deletePdfPagesConfig.content?.[language] || deletePdfPagesConfig.content?.en || {};
  }, [language]);

  const seoData = useMemo(() => {
    return deletePdfPagesConfig.seo?.[language] || deletePdfPagesConfig.seo?.en || {};
  }, [language]);
  ...
  const toolCategory = deletePdfPagesConfig.metadata?.category || null;
  ...
  const structuredData = useMemo(() => {
    return {
      webApplication: generateWebApplicationSchema(deletePdfPagesConfig, language),
      howTo: generateHowToSchema(deletePdfPagesConfig, language),
      faq: generateFAQSchema(deletePdfPagesConfig, language)
    };
  }, [language]);
