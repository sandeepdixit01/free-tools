...
const ImageCompressor = () => {
    ...
    if (process.env.NODE_ENV === 'development') {
      if (imageCompressorConfig.metadata?.schemaVersion === '2.0.0') {
        ...
        if (!result.isValid) {
          ...
          console.log('✅ ImageCompressor config is valid (Schema v2.0)');
        }
      }
    }
  }, []);
  ...
  const content = useMemo(() => {
    return imageCompressorConfig.content?.[language] || imageCompressorConfig.content?.en || {};
  }, [language]);

  const seoData = useMemo(() => {
    return imageCompressorConfig.seo?.[language] || imageCompressorConfig.seo?.en || {};
  }, [language]);
  ...
  const toolCategory = imageCompressorConfig.metadata?.category || null;
  ...
  const structuredData = useMemo(() => {
    return {
      webApplication: generateWebApplicationSchema(imageCompressorConfig, language),
      howTo: generateHowToSchema(imageCompressorConfig, language),
      faq: generateFAQSchema(imageCompressorConfig, language)
    };
  }, [language]);
