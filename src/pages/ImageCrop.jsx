...
const ImageCrop = () => {
    ...
    if (process.env.NODE_ENV === 'development') {
      if (imageCropConfig.metadata?.schemaVersion === '2.0.0') {
        ...
        if (!result.isValid) {
          ...
          console.log('✅ ImageCrop config is valid (Schema v2.0)');
        }
      }
    }
  }, []);
  ...
  const content = useMemo(() => {
    return imageCropConfig.content?.[language] || imageCropConfig.content?.en || {};
  }, [language]);

  const seoData = useMemo(() => {
    return imageCropConfig.seo?.[language] || imageCropConfig.seo?.en || {};
  }, [language]);
  ...
  const toolCategory = imageCropConfig.metadata?.category || null;
  ...
  const structuredData = useMemo(() => {
    return {
      webApplication: generateWebApplicationSchema(imageCropConfig, language),
      howTo: generateHowToSchema(imageCropConfig, language),
      faq: generateFAQSchema(imageCropConfig, language)
    };
  }, [language]);
