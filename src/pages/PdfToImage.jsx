...
const PdfToImage = () => {
    ...
    if (process.env.NODE_ENV === 'development') {
      if (pdfToImageConfig.metadata?.schemaVersion === '2.0.0') {
        ...
        if (!result.isValid) {
          ...
          console.log('✅ PdfToImage config is valid (Schema v2.0)');
        }
      }
    }
  }, []);
  ...
  const content = useMemo(() => {
    return pdfToImageConfig.content?.[language] || pdfToImageConfig.content?.en || {};
  }, [language]);

  const seoData = useMemo(() => {
    return pdfToImageConfig.seo?.[language] || pdfToImageConfig.seo?.en || {};
  }, [language]);
  ...
  const toolCategory = pdfToImageConfig.metadata?.category || null;
  ...
  const structuredData = useMemo(() => {
    return {
      webApplication: generateWebApplicationSchema(pdfToImageConfig, language),
      howTo: generateHowToSchema(pdfToImageConfig, language),
      faq: generateFAQSchema(pdfToImageConfig, language)
    };
  }, [language]);
