...
const TextCaseConverter = () => {
    ...
    if (process.env.NODE_ENV === 'development') {
      if (textCaseConverterConfig.metadata?.schemaVersion === '2.0.0') {
        ...
        if (!result.isValid) {
          ...
          console.log('✅ TextCaseConverter config is valid (Schema v2.0)');
        }
      }
    }
  }, []);
  ...
  const seoData = useMemo(() => {
    return textCaseConverterConfig.seo?.[language] || textCaseConverterConfig.seo?.en || {};
  }, [language]);
  ...
  const toolCategory = textCaseConverterConfig.metadata?.category || null;
