...
const MergePdf = () => {
    ...
    if (process.env.NODE_ENV === 'development') {
      if (mergePdfConfig.metadata?.schemaVersion === '2.0.0') {
        ...
        if (!result.isValid) {
          ...
          console.log('✅ MergePdf config is valid (Schema v2.0)');
        }
      }
    }
  }, []);
  ...
  const content = useMemo(() => {
    return mergePdfConfig.content?.[language] || mergePdfConfig.content?.en || {};
  }, [language]);

  const seoData = useMemo(() => {
    return mergePdfConfig.seo?.[language] || mergePdfConfig.seo?.en || {};
  }, [language]);
  ...
  const toolCategory = mergePdfConfig.metadata?.category || null;
    ...
    <>
      ...
      <ToolLayout
        ...
        heroComponent={
          <ToolHero
            ...
            onCtaClick={() => document.getElementById('file-upload-input')?.click()}
          />
        }
      ...
      >
        ...
        <div className="tool-content">
          ...
          {/* Merge PDF Controls (File Upload + Merge + Download) */}
          <MergePdfControls
