...
const RotatePdf = () => {
    ...
    if (process.env.NODE_ENV === 'development') {
      if (rotatePdfConfig.metadata?.schemaVersion === '2.0.0') {
        ...
        if (!result.isValid) {
          ...
          console.log('✅ RotatePdf config is valid (Schema v2.0)');
        }
      }
    }
  }, []);
  ...
  const content = useMemo(() => {
    return rotatePdfConfig.content?.[language] || rotatePdfConfig.content?.en || {};
  }, [language]);

  const seoData = useMemo(() => {
    return rotatePdfConfig.seo?.[language] || rotatePdfConfig.seo?.en || {};
  }, [language]);
  ...
  const toolCategory = rotatePdfConfig.metadata?.category || null;
  ...
  const structuredData = useMemo(() => {
    return {
      webApplication: generateWebApplicationSchema(rotatePdfConfig, language),
      howTo: generateHowToSchema(rotatePdfConfig, language),
      faq: generateFAQSchema(rotatePdfConfig, language)
    };
  }, [language]);
