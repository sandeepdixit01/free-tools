 ...
 * Text-based tool for removing duplicate lines
...
const RemoveDuplicateLines = () => {
    ...
    if (process.env.NODE_ENV === 'development') {
      if (removeDuplicateLinesConfig.metadata?.schemaVersion === '2.0.0') {
        ...
        if (!result.isValid) {
          ...
          console.log('✅ RemoveDuplicateLines config is valid (Schema v2.0)');
        }
      }
    }
  }, []);
  ...
  const seoData = useMemo(() => {
    return removeDuplicateLinesConfig.seo?.[language] || removeDuplicateLinesConfig.seo?.en || {};
  }, [language]);
  ...
  const toolCategory = removeDuplicateLinesConfig.metadata?.category || null;
  ...
  const structuredData = useMemo(() => {
    return {
      webApplication: generateWebApplicationSchema(removeDuplicateLinesConfig, language),
      howTo: generateHowToSchema(removeDuplicateLinesConfig, language),
      faq: generateFAQSchema(removeDuplicateLinesConfig, language)
    };
  }, [language]);
